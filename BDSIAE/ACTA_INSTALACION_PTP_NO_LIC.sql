IF OBJECT_ID('ACTA_INSTALACION_PTP_NO_LIC') IS NOT NULL
DROP PROC ACTA_INSTALACION_PTP_NO_LIC
GO
CREATE PROC ACTA_INSTALACION_PTP_NO_LIC
@CH_ID_TAREA CHAR(7)
AS
	BEGIN
...
-------------------------------------------------------------------------------
	...
	--VALORES FUERA DEL FORMULARIO 
			...
			ON T.VC_ID_NODO_IIBB_A = IPP.CH_ID_NODO_A
			WHERE T.CH_ID_TAREA =@CH_ID_TAREA) AS DEFAULT_GATE_A,
			...
			(SELECT CH_ID_NODO_A,VC_DIS_FREC_NODO_A
			FROM PTP_DET
			...
			(SELECT P.NU_COTA_NODO_A
			FROM PTP P INNER JOIN TAREA T 
			ON P.CH_ID_NODO_A = T.CH_ID_NODO_B
