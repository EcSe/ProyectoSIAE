 ...
 */
    ...
    define(/** @lends Constants */ 'Constants', [], function () {
        ...
        var Constants = {
            ...
            events: {
                ...
                removeAllFiles: 'remove_all_files',
    ...
    define(/** @lends Translator */ 'i18n', [], function () {
        var LOCALE = {
            ...
            'es_ES': {
                ...
                'Select Files or Drag&Drop Files': 'Seleccionar o Arrastrar y Soltar Archivos',
    ...
    define('ExifReader', ['Utils'], /** @lends ExifReader */ function (Utils) {
        ...
        var ExifReader = function (file, options) {
            var me = this;
            //default options
            //console.log("ExifReader.start file= " + file);
            //console.log("ExifReader.start file.imgURL = " + file.imgURL);
            me.options = {
        ...
        ExifReader.prototype = {
            start: function () {
                ...
                //console.log("ExifReader.start01");
    ...
    function (Constants, Utils, SimpleRunner, _, FileMd5, ExifReader, ImageScale) {
        ...
        var FileObject = function (file, fileId, AU, imgURL) {
            ...
            //console.log("FileObject01 me.disabled = " + me.disabled);
        ...
        FileObject.prototype = {
            ...
            init: function () {
                var me = this;
                //console.log("init 01 me.disabled = " + me.disabled);
                //visual part
                //console.log("init 01");
                //alert("init 01");
                me.renderHtml();
                ...
                //console.log("init 03");
                //console.log("me = " + me);
                me.bindFilePreview();//Muestra el preview del archivo subido
                //console.log("init 05 me.disabled = " + me.disabled);
                //run the queue operations
                me.disableUpload(_('Processing'));
                //console.log("init 06 me.disabled = " + me.disabled);
                me.runQueue();
                //console.log("init 07 me.disabled = " + me.disabled);
                return me;
            },
            /**
            ...
            destroy: function () {
                ...
                var fileNamesUploaded = []; //collect file names in a array
                ...
                for (var fileId in me.AU.fileList) {
                    if (me.AU.fileList.hasOwnProperty(fileId)) {
                        fileNames.push(me.AU.fileList[fileId].name);
                        if (me.AU.fileList[fileId].status == 1) fileNamesUploaded.push(me.AU.fileList[fileId].name);
                    }
                }

                me.AU.triggerEvent('removeFile', [fileNames, me.AU.fileList, fileNamesUploaded]);
            ...
            stopQueue: function () {
                ...
                //console.log("stopQueue 01");
                this.enableUpload(_('Listo para subir'));
                //console.log("stopQueue 01");
                this._preUploadFun = [];
            },
            /**
            ...
            runQueue: function () {
                ...
                if (this._preUploadFun.length > 0) {
                    ...
                    //this.enableUpload(_('Ready for upload'));
                    //console.log("runQueue 01");
                    this.enableUpload(_('Listo para subir'));
                    //console.log("runQueue 02");
                }
                return this;
            },
            /**
            ...
            renderHtml: function () {
                var me = this;
                //console.log("FileObject.renderHtml 01");
                ...
                //console.log("renderHtml01 me.dom.removeButton = " + me.dom.removeButton);
                ...
                me.setName(me.name)
                    ...
                    .setRemoveButton(_('Eliminar archivo de la cola'))
                    .setInfoButton();
            ...
            setUploadButton: function () {
                if (this.dom.uploadButton && !this.config.hideUploadButton) {
                    if (this.status == Constants.AX_READY || this.status == Constants.AX_ERROR) {
                        ...
                        this.dom.uploadButton.setAttribute('title', _('Cancelar carga'));
                        if (this.dom.uploadButtonText) {
            ...
            setDeleteButton: function (enable) {
                if (this.dom.deleteButton) {
                    if (enable) {
                        ...
                        this.dom.deleteButton.setAttribute('title', _('Eliminar archivo del servidor'));
                        ...
                        this.dom.deleteButton.setAttribute('title', _('Archivo aún por cargar'));
            ...
            setInfoButton: function () {
                if (this.dom.infoButton) {
                    ...
                    this.dom.infoButton.setAttribute('title', _('Mostrar información del archivo'));
            ...
            disableUpload: function (msg) {
                var me = this;
                //console.log("disableUpload 01 me.disabled = " + me.disabled);
                me.disabled = true;
                //console.log("disableUpload 02 me.disabled = " + me.disabled);
                //console.log("disableUpload 03 msg = " + msg);
            ...
            enableUpload: function (msg) {
                var me = this;
                //console.log("enableUpload 01 me.disabled = " + me.disabled);
                me.disabled = false;
                //console.log("enableUpload 02 me.disabled = " + me.disabled);
                //Agregado Carlos Ramos 12/02/2018 validar dimensiones de la imagen subida
                //Si no pasa la validacion el control me.dom viene nulo
                //Inicio
                //me.dom.container.classList.remove('ax-disabled');
                if (me.dom != null) me.dom.container.classList.remove('ax-disabled');
                //Fin
                me.setMessage(msg);
                me.setProgress(0);
                return me;
            ...
            setProgress: function (percent) {
                //Agregado Carlos Ramos 12/02/2018 validar dimensiones de la imagen subida
                //Si no pasa la validacion el control this.dom viene nulo
                //Inicio
            ...
            workProgress: function (msg, p) {
                var me = this;
                me.setProgress(p);
                //console.log("workProgress cramos01");
                //console.log("workProgress p = " + p);
                //console.log("workProgress msg = " + msg);
                me.setMessage(msg + ' ' + p + '%');
                //Agregado Carlos Ramos 12/02/2018 validar dimensiones de la imagen subida
                //Si no pasa la validacion el control me.AU viene nulo
                //Inicio
                //me.AU.triggerEvent('progressFile', [this, p]);
                if (me.AU != null) me.AU.triggerEvent('progressFile', [this, p]);
                //Fin
                return me;
            },
            /**
            ...
            setMessage: function (msg) {
                var me = this;
                //console.log("setMessage cramos01");
                //console.log("msg = " + msg);
                //console.log("me.dom.progressInfo = " + me.dom.progressInfo);
                //console.log("setMessage 01 me = " + me);
                //console.log("setMessage 02 me.dom = " + me.dom);
                //console.log("setMessage 03 (me.dom != null) = " + (me.dom != null));
                //console.log("setMessage 04 me.dom.progressInfo = " + me.dom.progressInfo);
            ...
            setStatus: function (status) {
                ...
                switch (me.status) {
                    ...
                    case Constants.AX_READY:
                        ...
                        me.workProgress(_('Listo para subir'), 0);
                    ...
                    case Constants.AX_DONE:
                        ...
                        me.setMessage(_('Archivo subido'));
            ...
            bindEvents: function () {
                ...
                if (me.dom.removeButton) {
                    me.dom.removeButton.addEventListener('click', function () {
                        ...
                        //console.log("me.dom.removeButton.click");
                        //console.log("me.dom.removeButton.click me.config.enable = " + me.config.enable);
                        //console.log("me.dom.removeButton.click me.disabled = " + me.disabled);
                        if (me.config.enable && !me.disabled) {
                            me.destroy();
                        } else {
                            me.stopQueue();
                ...
                if (me.dom.deleteButton && me.config.allowDelete) {
                    me.dom.deleteButton.addEventListener('click', function () {
                        if (!me.disabled && me.status === Constants.AX_DONE) {
                            me.askUser(_('Eliminar archivo cargado?')).yes(function () {
                                me.deleteFile();
                            }).no(function () {
                ...
                if (this.dom.infoButton) {
                    this.dom.infoButton.addEventListener('click', function () {
                        //console.log("this.dom.infoButton.click cramosc01");
                        var strPretty = '';
                        //console.log("this.dom.infoButton.click me.exifData = " + me.exifData);
            ...
            bindFilePreview: function () {
                //console.log("FileObject.bindFilePreview 01 this = " + this);
                var me = this;
            ...
            askUser: function (msg) {
                ...
                var html = ['<div class="ax-ask-inner">',
                                ...
                                '<a title="' + _('Yes') + '" class="ax-button ax-reply-yes"><span class="ax-icon"></span> <span class="ax-btn-text">' + _('Si') + '</span></a>',
            ...
            startUpload: function () {
                ...
                //console.log("startUpload01 me.status = " + me.status);
            ...
            _upload: function () {
                ...
                me.xhr.upload.addEventListener('progress', function (e) {
                    if (e.lengthComputable) {
                        ...
                        //console.log("xhr.upload.progress cramos01");
                        //console.log("xhr.upload.progress e.loaded = " + e.loaded);
                        //console.log("xhr.upload.progress currentByte = " + currentByte);
                        //console.log("xhr.upload.progress size = " + size);
                        //console.log("xhr.upload.progress progress = " + progress);
                        me.workProgress(_('Cargando'), progress);
                ...
                me.xhr.upload.addEventListener('error', function (e) {
                    //console.log("xhr.upload.error cramos01");
                    me._onError(this.responseText);
                    //console.log("xhr.upload.error cramos02");
                }, false);

                me.xhr.onreadystatechange = function () {
                    //console.log("me.xhr.onreadystatechange cramosc01");
                    if (this.readyState == 4 && this.status == 200) {
                        //console.log("me.xhr.onreadystatechange cramosc02");
                        me.AU.triggerEvent('chunkUpload', [file, name, chunk, me.xhr]);
                        //console.log("me.xhr.onreadystatechange cramosc03");
                        try {
                            //console.log("me.xhr.onreadystatechange cramosc04");
                            //console.log("me.xhr.onreadystatechange this.responseText = " + this.responseText);
                            var ret = JSON.parse(this.responseText);
                            //console.log("me.xhr.onreadystatechange cramosc05");
                            ...
                            if (currentByte == 0) {
                                //console.log("me.xhr.onreadystatechange cramosc06");
                                me.setTempName(ret.temp_name);
                                //console.log("me.xhr.onreadystatechange cramosc07");
                            }
                            //console.log("me.xhr.onreadystatechange cramosc08");
            ...
            _onStart: function () {
                ...
                me.workProgress(_('Subida iniciada'), 0);
                me.loading_bytes = 0;
            ...
            _onFinishUpload: function (json) {
                ...
                me.setName(json.name)
                    ...
                    .setMessage(_('Archivo subido'));//update view information
            ...
            _onComplete: function () {
                ...
                if (me.AU != null) me.AU.fileCompleted(me.fileId);
                //Fin
                return this;
            }
        };
    ...
    define('RealUploader', ['FileObject', 'Constants', 'Utils', 'i18n'], /** @lends RealUploader */ function (FileObject, Constants, Utils, _) {
        ...
        var RealUploader = function (querySelector, config) {
            ...
            this.events = {
                ...
                removeAllFiles: [], //se ejecuta despues de presionar el boton quitar todos
                removeFile: [] //se ejecuta despues de presionar el boton quitar archivo
            };
        ...
        RealUploader.prototype = {
            ...
            renderHtml: function () {
                ...
                //console.log("RealUploader.renderHtml01");
            ...
            _bindEvents: function () {
                ...
                if (this.dom.browseInput) {
                    this.dom.browseInput.addEventListener('change', function () {
                        //console.log("browseInput.change.01");
                        //console.log("browseInput.change.01 this.files = " + this.files);
                ...
                if (this.dom.uploadButton) {
                    this.dom.uploadButton.addEventListener('click', function () {
                        //console.log("me.config.enable = " + me.config.enable);
            ...
            fileCompleted: function (fileId, msg) {
                ...
                for (var fid in this.fileList) {
                    if (this.fileList.hasOwnProperty(fid)) {
                        ...
                        if (f.status == 1) fileNamesUploaded.push(f.name);
                    }
                }

                switch (fileObj.status) {
                    case Constants.AX_DONE:
                        this.triggerEvent('finishFile', [fileObj, msg, fileNames, fileNamesUploaded]);
                        break;
                    case Constants.AX_ERROR:
            ...
            finish: function () {
                ...
                var fileNames = []; //collect file names in a array
                //console.log("finish01 this.fileList.length = " + this.fileList);
                for (var fileId in this.fileList) {
                    if (this.fileList.hasOwnProperty(fileId)) {
                        //console.log("finish fileId = " + fileId);
                        fileNames.push(this.fileList[fileId].name);
                    }
                }
                //console.log("finish fileNames = " + fileNames);
                //console.log("finish this.fileList.length = " + this.fileList.length);
            ...
            addFiles: function (files) {
                //console.log("addFiles01");
                //console.log("files = " + files);
                var selectedFiles = [];//store this just for the on select event

                //console.log("RealUploader.addFiles01 files.length = " + files.length);
            ...
            addEmptyFile: function (imgURL, fileName) {
                ...
                //add selected files to the queue
                //for (var i = 0; i < files.length; i++) {
                //El segundo parametro es el nombre del archivo
                //vamos a usar la URL como nombre para que no se caiga el metodo checkExt()
                //del archivo Upload.aspx.cs
            ...
            checkFile: function (name, size, ext) {
                ...
                //console.log("checkFile01 me.config.isImage = " + me.config.isImage);
                //console.log("checkFile01 ext = " + ext);
                //console.log("checkFile01 file = " + file);
             ...
             */
            enqueueAll: function () {
                //console.log("enqueueAll");
                var pending = this.getPendingFiles();
                //console.log("enqueueAll01 pending.length = " + pending.length);
                ...
                if (pending.length == 0) {
                    ...
                    //on start callback
                    //console.log("enqueueAll cramosc01");
                    //console.log("antes de lanzar el evento start");
                    //console.log("this" + this);
                    this.triggerEvent('start', [pending]);
                    //console.log("despues de lanzar el evento start");
                    ...
                    me.checkInterval = setInterval(function () {
                        ...
                        if (me.uploadQueue.length == 0) {
                            ...
                            for (var i = 0; i < me.uploadQueue.length; i++) {
                                ...
                                //console.log("processQueue01 file.status = " + file.status);
                                //console.log("processQueue01 me.slots = " + me.slots);
                                //console.log("processQueue01 Constants.AX_READY = " + Constants.AX_READY);
                                //console.log("processQueue01 file.disabled = " + file.disabled);
                                ...
                                //console.log("processQueue02 file.status = " +file.status);
                            }
                        }
                    }, 500);
                }
            ...
            getPendingFiles: function () {
                ...
                for (var fileId in this.fileList) {
                    if (this.fileList.hasOwnProperty(fileId)) {
                        ...
                        //console.log("getPendingFiles01 f.status = " + f.status);
            ...
            clearQueue: function () {
                ...
                //Llamar al evento para saber que se han eliminado los archivos
                this.triggerEvent('removeAllFiles');
            },
            ...
            enable: function (bool) {
                //console.log("enable01 this.config.enable = " + this.config.enable);
                this.config.enable = bool;
                //console.log("enable02 this.config.enable = " + this.config.enable);
                return this;
            },
